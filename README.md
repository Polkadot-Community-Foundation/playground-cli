...
- `@parity/product-sdk-*` packages are pinned to **exact versions** (no `^`). The SDK is pre-1.0 and any patch bump can flip wire shape; pinning makes bumps intentional and lockfile-visible. Aligning every package to its registry-latest at install time is the goal — we just want it explicit. CI's `Format` job runs a grep guard that fails the build on any direct `@polkadot-apps/*` import in `src/`, `e2e/`, `scripts/`, or `tools/`.
