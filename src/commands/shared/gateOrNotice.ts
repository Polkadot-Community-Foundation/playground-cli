 ...
 *
 * PCF fork: the builder-identity gate is a competition mechanic. Our official
 * seed apps are deployed by the operator key (5Fk8) from headless CI with no
 * login session, so the gate would soft-no-op them. Setting
 * `PLAYGROUND_SKIP_IDENTITY_GATE=1` opts those operator deploys out explicitly.
 * The gate stays fully enforced for every interactive/competition user.
 */
export async function enforceIdentityGate(
    rawAssetHubClient: PolkadotClient,
    registry?: IdentityRegistry,
): Promise<boolean> {
    const skip = process.env.PLAYGROUND_SKIP_IDENTITY_GATE;
    if (skip && skip !== "0" && skip.toLowerCase() !== "false") return false;
