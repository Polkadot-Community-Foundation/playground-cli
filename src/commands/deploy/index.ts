...
export const deployCommand = new Command("deploy")
    ...
    .action(async (opts: DeployOpts) =>
        runCliCommand("deploy", { watchdog: true, hardExit: true }, async () => {
            ...
            // error behind a soft-block hid it entirely. Then make --env the
            // process-wide active env BEFORE any chain access, so the no-arg
            // getConnection()/getChainConfig() paths (registry publish, account
            // mapping) follow it instead of defaulting to Paseo.
            getChainConfig(env);
            setActiveEnv(env);
