...
- **No `@novasamatech/*` pnpm overrides — the tree resolves natively.** `@parity/product-sdk-terminal@0.3.0` declares `^0.8.1`, so `@novasamatech/host-papp`, `statement-store`, and `storage-adapter` resolve to single 0.8.4 copies with no forcing. The old `0.7.9-4` overrides are gone; do NOT re-add them. Upstream RENAMED `@novasamatech/product-sdk` → `@novasamatech/host-api-wrapper` (triangle-js-sdks #169, no compat re-export) — it's gone from the tree and nothing in our code may depend on the old name. Residual `host-api@0.7.9` / `host-api-wrapper@0.7.9` / `scale@0.7.9` copies exist ONLY under the `@dotdm/contracts@^3.x` subtree (immovable from our side; dormant, signer-less paths) and go away once @dotdm bumps to `@parity/product-sdk-contracts` 0.7. novasama 0.8 is wire-incompatible with 0.7: sessions paired by an older CLI build can't be decoded, so `auth.ts::loadSessions` translates decode failures into a `STALE_SESSION_MESSAGE` "playground logout / playground init" hint rather than a raw throw.
